...
const FriendCard = styled('div')<{
  ...
  groupColor?: string;
  noFriendsGroup: boolean;
}>(({ selected, partiallySelected, groupColor, noFriendsGroup, theme }) => ({
  padding: '16px',
  border: '2px solid',
  display: noFriendsGroup ? 'none' : 'div',
...
const StatusBadge = styled('div')<{ status: 'full' | 'partial' | 'none' }>(
  ({ status, theme }) => ({
    fontSize: '11px',
    padding: '4px 8px',
    borderRadius: '8px',
    ...
    backgroundColor:
      status === 'full'
        ...
        : status === 'partial'
          ...
          : '#f5f5f5',
    color:
      status === 'full' ? '#2e7d32' : status === 'partial' ? '#f57c00' : '#666',
...
const todayFriendCard: React.FC<FriendCardProps> = ({
  ...
  const handleGroupClick = (group: GetUserFriendByGroupResponse) => {
    ...
    if (isFullySelected) {
      // 그룹의 모든 친구 제거
      const friendsToRemove = group.friend_link_group.map(
        (linkGroup) => linkGroup.friend.id,
      );
  ...
  return (
    <GridContainer>
      {friendGroups.map((group) => {
        // organizeByGroups() 제거하고 직접 friendGroups 사용
        const isFullySelected = isGroupFullySelected(group);
        const isPartiallySelected = isGroupPartiallySelected(group);
        const selectedCount = selectedFriends.filter(
          (selected) =>
            group.friend_link_group.some(
              (linkGroup) => linkGroup.friend.id === selected.id,
            ), // 수정
        ).length;
        const isNoFriendsGroup =
          group.friend_link_group.length === 0 ? true : false;
        ...
        return (
          ...
          >
            ...
            {/* 그룹 내 친구들 미리보기 */}
            <FriendPreview>
              {group.friend_link_group.slice(0, 3).map(
                (
                  linkGroup, // 수정
                ) => (
              ...
              {group.friend_link_group.length > 3 && ( // 수정
                <MoreFriends>
                  +{group.friend_link_group.length - 3}명 더
                </MoreFriends>
              )}
            </FriendPreview>
            ...
            <SelectionStatus>
              ...
              {isFullySelected && !isNoFriendsGroup && (
                <StatusBadge status="full">✅ 모든 친구 선택됨</StatusBadge>
              )}
              {isPartiallySelected && (
                <StatusBadge status="partial">
                  ⚪ {selectedCount}/{group.friend_link_group.length} 선택됨{' '}
                  {/* 수정 */}
                </StatusBadge>
              )}
